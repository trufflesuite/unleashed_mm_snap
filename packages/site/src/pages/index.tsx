...
const Index = async () => {
  ...
  const handleChainChanged = async (_chainId:number) => {
    window.location.reload();
  }
  window.ethereum.on('chainChanged', handleChainChanged as (...args: unknown[]) => void);

  const chainId = await window.ethereum.request({ method: 'eth_chainId' });
  const simpleNFTContractAddress = SimpleNFT.networks[chainId].address;
  const simpleNFTInterface = new ethers.utils.Interface(SimpleNFT.abi);
  const nftVaultContractAddress = NFTVault.networks[chainId].address;
  const nftVaultInterface = new ethers.utils.Interface(NFTVault.abi);
